# Vitis Extensible Platform Design for Streaming Applications on Kria KV260
Author: Frank Kesel, 23.01.2026

Credits to A.Schmid and M.Gaiser, who contributed to this document.

---
- [Vitis Extensible Platform Design for Streaming Applications on Kria KV260](#vitis-extensible-platform-design-for-streaming-applications-on-kria-kv260)
  - [Overview](#overview)
  ...
  - [Install Sysroot directory](#install-sysroot-directory)
  - [Links](#links)
...
* The development described here has been tested with Vivado/Vitis Versions 2025.1. Depending on your installation you may have to adapt paths (e.g. paths to the Xilinx installation on your computer) in the scripts etc.
...
* So far we have designed the extensible platform with Vivado and exported it as `.xsa`-file. For application projects in Vitis we need to generate Vitis platform data out of the exported platform, this is called a `platform project` in Vitis. This will be done in the following steps with Vitis SDK. The generated platform project can then be referenced in Vitis application projects (see other tutorials). The platform project has to be generated only once and can serve different Vitis application projects. For the following steps and also the Vitis application projects it is assumed that you have a Vitis installation under Linux (Ubuntu 22.04 or 24.04).
1. First we need to create some folders to store platform data. Make a folder named `kv260_vitis_platform`, preferably in the same workspace/folder where the Vivado project resides.  Open a Linux shell and go to the folder  `kv260_vitis_platform`. Then do the following:
   * `mkdir pfm`
   * `mkdir pfm/boot`
   * `mkdir pfm/sd_dir`
   * The boot and sd_dir folders will be left empty in the following, since we have already a running Ubuntu Linux system on the Kria target.
2. Open Vitis and select the folder `kv260_vitis_platform` as a workspace.
3. Create a new platform project
   * Select menu `File > New Component > Platform` to create a platform project.
   * Enter the project name. Name the platform e.g. `kv260_platform`. Click `Next`.
   * Select `Hardware Design`. Select the XSA file generated by the Vivado. In this case, it is `kv260_hw_platform.xsa` generated in the steps above. Click `Next`.
   * Set the operating system to `linux`.
   * Set the processor to `psu_cortexa53` and the architecture to 64-bit.
    * Uncheck option `Generate boot artifacts` because we have already an up and running Ubuntu Linux on the target.
    * Click `Finish`.
4. Setup software settings in Platform Settings view (`vitis_comp.json`).
   * Click the `linux on psu_cortexa53` domain, browse to the locations and select the directory or file needed to complete the dialog box for the following:
     * Bif file: Click the drop-down icon next to the `Browse` button and select `Generate BIF`.
   * FAT32 Partition Directory: Browse to `kv260_vitis_platform/pfm/sd_dir` and click `OK`.     
   * Leave the other fields as they are.
5. In the `Flow` pane, click the `Build` button to generate the platform.
...
---
## Install Sysroot directory
* For applications using the platform generated above a so-called `sysroot` directory is needed. This is mainly needed when we build C/C++ programs for the target system. The sysroot directory contains headers and libraries needed for the build process. The host headers and libraries can not be used here, since we build the program for the target system. Therefore the sysroot for the target system is needed. The generation of the sysroot directory must only be done once. Instead of processing the steps below you can download a sysroot-directory from [here](https://itwww.hs-pforzheim.de/daten/mitarbeiter/kesel/rs_lab/kv260_sysroot_22_2.tar.gz)
1. For the following steps we need to download the `common image` for the ZynqMP-SOC which is a prebuilt Linux kernel and root file system. Since we have already Ubuntu Linux on the Kria board, only the `sysroot` will be needed for cross-compilation and linking the program against the libraries of the target Linux system, as mentioned above.  
2. Create a folder named `kv260_sysroot` in the workspace. 
3. Download `common image` (Petalinux image for ZynqMP-SOC) from https://www.xilinx.com/support/download/index.html/content/xilinx/en/downloadNav/embedded-platforms.html
   * Select the version fitting your Vitis installation (e.g. 2022.2)
   * Select `ZYNQMP common image`
   * Store the file in the workspace
4. Go to the workspace and extract the common image (in a Linux shell): `tar xvf ../xilinx-zynqmp-common-v2022.2_10141622.tar.gz -C .` (adapt the name if necessary). Be aware that the extraction will consume considerable disk space (several GBs). You should now have a directory `xilinx-zynqmp-common-v2022.2` inside the workspace.
5. Install `sysroots`: 
   * Go to `xilinx-zynqmp-common-v2022.2` directory.
   * Type `./sdk.sh -d <Install Target Dir>` to install `sysroots`. use the -d option to provide a full pathname to the output directory `kv260_sysroot` and confirm.
   * Note: The environment variable LD_LIBRARY_PATH must not be set when running this command
   * After extraction you should now have a folder `sysroots` in the folder `kv260_sysroot`
   * The only directory needed is `kv260_sysroot/sysroots/cortexa72-cortexa53-xilinx-linux`. You can copy this directory to another space and remove all the rest of the directories in the workspace (which you should do because they consume a lot of space). The mentioned sysroots directory needs to be referenced in Vitis application projects, so it is a good idea to copy it to the same place where you stored the Vitis platform (see above). 
   


---
## Links
* https://github.com/Xilinx/Vitis-Tutorials/tree/2022.1/Vitis_Platform_Creation/Design_Tutorials/01-Edge-KV260
* https://docs.xilinx.com/r/en-US/ug1393-vitis-application-acceleration/Embedded-Processor-Application-Acceleration-Development-Flow
