...
* Open a Linux terminal, go to the folder `tutorial_03` and execute the bash script `sol1.sh`. Then start the Vitis GUI and open the workspace `tutorial_03`. Since synthesis has already been run you can open the synthesis report as it is shown in the following image. 
...
* Before you start a synthesis run, check the configuration file (in the _Source Editor_). Keep in mind that this file has the same name as in `sol1` but is different since it is stored in the directory `sol2`. You should have an entry `syn.directive.pipeline=matmul/Product` which tells you that the loop `Product` will be pipelined. Run the synthesis hand open the synthesis report. You should see the result similar to the following image.
...
*  Compare the reports for both solutions (`View > HLS Compare Reports`) as shown in the image below. You can see that we reduced the interval from 161 clock cycles to 33 clock cycles, which is a factor of 4.9! When you compare the resources you can see that the number of DSPs is still the same, but we need more flipflops and LUTs. But this was a very effective optimization since we could reduce the latency and interval considerably. What is important is the fact that the number of DSPs, which is a precious resource, did not increase, which follows from the fact that the loops were not unrolled. 
...
* Open the source code `matmul.cpp` and open the _HLS Directives_ viewer. If you still see the pipeline directive from solution `sol2` then look into the configuration if the pipeline directive is there (e.g. when you cloned accidentally `sol2`) and remove it. Select the loop `Col` and apply again the `PIPELINE` directive to the loop (store it in the config file) as shown in the next image. Set the `II` value (Initiation Interval) for the loop to 1. In the configuration file you should now have the directive `syn.directive.pipeline=matmul/Col II=1`.
...
* Add the directive `ARRAY_PARTITION` to the config file as shown in the next image. The `dim` value (dimension) must be set to 2 and the `factor` to 2. This will partition the array along dimension 2 by a factor of 2. The type must be set to `cyclic`. 

![Vitis directives](images/hls_82.png)

*  When you run synthesis you should still see an II violation, but when you look in the problem report you can see that now array `b` is the problem. In the synthesis report you can see that we have now 3 ports for array `a` but only 2 ports for array `b`, so array `b` is still a bottleneck.
...
*  You may wonder what the meaning of the `dim` value is and why it was set to 2 for array `a` and 1 for array `b`? For this example it is important that you partition the array along the appropriate dimension. Look into the code: The inner loop `Product` iterates over loop variable `k` and since this loop is unrolled we have to parallelize the array accesses for variable `k`. In the code you see that `temp += a[i][k] * b[k][j];`, where `a` has variable `k` in the second dimension and `b` has `k` in the first dimension. 
*  You can verify that the dimension is important by changing for example the directive for `b` from 1 to 2 and running synthesis: You get a similar result as without partitioning the array as shown in the next image. What is even worse is the fact that now two dual port interfaces for `b` have been generated but we have no benefit in terms of performance.   
